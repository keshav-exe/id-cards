/**
 * Vendored from brandpull (MIT) — src/branding/colors.ts.
...
export function contrastYiq(hex2: string): number {
...
export function isGrayish(hex2: string): boolean {
  const h = hex2.replace("#", "");
  if (h.length < 6)
    return true;
  const r = Number.parseInt(h.slice(0, 2), 16);
...
export function isVibrant(hex2: string | null | undefined): boolean {
  if (!hex2)
    return false;
  const h = hex2.replace("#", "");
  if (h.length < 6)
