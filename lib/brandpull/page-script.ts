...
// @ts-nocheck
/**
 * Vendored verbatim from brandpull (MIT) — src/branding/page-script.ts.
 * https://github.com/suraj-xd/brandpull
 *
 * Runs INSIDE Chromium via `page.evaluate`. It must stay self-contained:
 * no imports, no references to module scope. Only DOM globals.
 */
import type { RawBranding } from "./types"

export function extractBrandingFromPage(): RawBranding {
  const constants = {
    buttonMinWidth: 50,
    buttonMinHeight: 25,
  ...
  const toPx = (value) => {
    ...
    if (value.endsWith("em"))
      return Number.parseFloat(value) * Number.parseFloat(getStyle(document.body ?? document.documentElement).fontSize || "16");
    if (value.endsWith("%"))
      return null;
    const parsed = Number.parseFloat(value);
    return Number.isFinite(parsed) ? parsed : null;
  };
  const textOf = (el, limit = 100) => (el?.textContent || "").trim().replace(/\s+/g, " ").slice(0, limit);
  ...
  const isHomeHref = (href) => {
    ...
    if (["", "/", "./", "/home", "/index", "/index.html", "#"].includes(trimmed))
      return true;
    if (trimmed.startsWith("#") || trimmed.startsWith("?"))
      return true;
    try {
      const url = new URL(trimmed, window.location.origin);
      if (!isSameBrandHost(window.location.hostname.toLowerCase(), url.hostname.toLowerCase()))
        return false;
      const path = url.pathname.replace(/\/$/, "") || "/";
      return path === "/" || path === "/home" || path === "/index" || path === "/index.html" || path.split("/").filter(Boolean).length === 1;
    } catch {
      const parts = trimmed.split("/").filter(Boolean);
      return parts.length === 1 && !trimmed.includes(".");
    }
  };
  const isExternalServiceHref = (href) => {
  ...
  const normalizeColor = (color) => {
    if (!color)
      return null;
    const normalized = color.toLowerCase().trim();
    if (!isValidBackgroundColor(normalized))
      return null;
    return normalized.replace(/\s+/g, "");
  };
  const collectCSSData = () => {
    const data = { colors: [], spacings: [], radii: [] };
  ...
  const isButtonElement = (el) => {
    if (!el || typeof el.matches !== "function")
      return false;
    const selector = 'button,input[type="submit"],input[type="button"],[role=button],[data-primary-button],[data-secondary-button],[data-cta],a.button,a.btn,[class*="btn"],[class*="button"],a[class*="bg-brand"],a[class*="bg-primary"],a[class*="bg-accent"]';
    ...
    const hasButtonClasses = /rounded(-md|-lg|-xl|-full)?|p[xy]?-\d+|border.*rounded|inline-flex.*items-center.*justify-center/.test(classes);
    const hasPadding = (Number.parseFloat(style.paddingTop) || 0) > constants.buttonMinPaddingVertical || (Number.parseFloat(style.paddingBottom) || 0) > constants.buttonMinPaddingVertical || (Number.parseFloat(style.paddingLeft) || 0) > constants.buttonMinPaddingHorizontal || (Number.parseFloat(style.paddingRight) || 0) > constants.buttonMinPaddingHorizontal;
    const hasSize = rect.width > constants.buttonMinWidth && rect.height > constants.buttonMinHeight;
    const hasShape = (Number.parseFloat(style.borderRadius) || 0) > 0 || (Number.parseFloat(style.borderTopWidth) || 0) > 0;
    return hasButtonClasses && hasSize || hasPadding && hasSize && hasShape;
  };
  const sampleElements = () => {
    const set = new Set;
    const push = (selector, limit) => {
      let count = 0;
    ...
    push('button, input[type="submit"], input[type="button"], [role=button], [data-primary-button], [data-secondary-button], [data-cta], a.button, a.btn, [class*="btn"], [class*="button"], a[class*="bg-brand"], a[class*="bg-primary"], a[class*="bg-accent"]', 120);
    for (const link of Array.from(document.querySelectorAll("a")).slice(0, 150)) {
      if (!set.has(link) && isButtonElement(link))
        set.add(link);
    }
  ...
  const getStyleSnapshot = (el) => {
    ...
    const classes = classString(el).toLowerCase();
    const fontStack = (style.fontFamily || "").split(",").map((font) => font.replace(/["']/g, "").trim()).filter(Boolean);
    ...
    const hasCTAIndicator = el.matches('[data-primary-button],[data-secondary-button],[data-cta],[class*="cta"],[class*="hero"]') || el.getAttribute("data-primary-button") === "true" || el.getAttribute("data-secondary-button") === "true";
    const isNavigation = !hasCTAIndicator && (/nav-|nav-|nav-link|sidebar|menu|toggle|trigger/.test(classes) || el.matches('[role="tab"],[role="menuitem"],[aria-haspopup],[aria-expanded]') || !!el.closest('nav, [role="navigation"], [role="menu"], [role="menubar"], [class*="navigation"], [class*="dropdown"], [class*="sidebar"], aside'));
    const isButton = isButtonElement(el) && !isNavigation;
    const isInput = el.matches('input:not([type="submit"]):not([type="button"]),select,textarea,[class*="form-control"]');
    const input = el;
    const text = tag === "input" && (input.type === "submit" || input.type === "button") ? (input.value || "").trim().slice(0, 100) : textOf(el, 100);
    ...
    return {
      ...
      colors: {
        ...
        background,
        border: borderTop === borderRight && borderTop === borderBottom && borderTop === borderLeft ? borderTop : borderTop,
        borderWidth: borderTopWidth === borderRightWidth && borderTopWidth === borderBottomWidth && borderTopWidth === borderLeftWidth ? borderTopWidth : borderTopWidth,
        borderTop,
        borderTopWidth,
        borderRight,
        borderRightWidth,
  ...
  const backgroundImageUrl = (value) => {
    if (!value || value === "none")
      return null;
    ...
    if (!simple)
      return null;
    return simple.trim().replace(/^["']|["']$/g, "").replace(/&quot;/g, '"').replace(/&lt;/g, "<").replace(/&gt;/g, ">");
  };
  ...
  const findImages = () => {
    ...
    const pushImage = (src, type) => {
      ...
      if (resolved)
        images.push({ type, src: resolved });
    };
    pushImage(document.querySelector('link[rel*="icon" i]')?.href, "favicon");
    pushImage(document.querySelector('meta[property="og:image" i]')?.content, "og");
    pushImage(document.querySelector('meta[name="twitter:image" i]')?.content, "twitter");
    const collect = (el, source) => {
      try {
        const rect = el.getBoundingClientRect();
        const style = getStyle(el);
        const parentLink = el.closest("a");
        const href = parentLink?.getAttribute("href") || "";
        if (href && isExternalServiceHref(href))
          return;
        const inHeader = !!el.closest('header, nav, [role="banner"], #navbar, [id*="navbar" i], [class*="navbar" i], [class*="globalnav" i], [role="menubar"]');
        ...
        const imgSrc = tag === "img" ? el.currentSrc || el.src : "";
        const isSvg = tag === "svg";
        const alt = tag === "img" ? (el.alt || parentAria || "").trim() : (ownAria || ownTitle || textOf(el, 80) || id || "").trim();
        const hasLogoData = /logo|brand|site-name|site-title/i.test(`${classes} ${id} ${ownAria} ${ownTitle} ${parentAria}`);
        ...
        if (/flag|language|locale/i.test(`${classes} ${id} ${alt}`) && rect.width <= 32 && rect.height <= 32)
          return;
        if (/search|magnif|hamburger|menu|cart|user|bell|chevron|arrow|caret|dropdown|close|settings/i.test(`${classes} ${id} ${alt}`) && !hasLogoData)
          return;
        if (!strongContext && !/logo|brand/i.test(`${imgSrc} ${bgUrl || ""} ${alt}`))
          return;
        let src = "";
        let finalSvg = false;
        let svgScore = 0;
        if (isSvg) {
          ...
          src = imageHref ? resolveUrl(imageHref) : serializeSvg(el);
    ...
    for (const selector of backgroundSelectors) {
      queryAllDeep(selector).forEach((el) => {
        if (backgroundImageUrl(getStyle(el).getPropertyValue("background-image")))
    ...
    queryAllDeep("img").forEach((img) => {
      const alt = img.alt || "";
      const src = img.src || "";
      if (/logo|brand/i.test(`${alt} ${src} ${classString(img)}`) && !img.closest('[class*="testimonial" i], [class*="client" i], [class*="partner" i], footer'))
        collect(img, "document.images");
    });
    queryAllDeep("svg").forEach((svg) => {
      const rect = svg.getBoundingClientRect();
      const existing = logoCandidates.some((c) => c.isSvg && Math.abs(c.position.top - rect.top) < 1 && Math.abs(c.position.left - rect.left) < 1 && Math.abs(c.position.width - rect.width) < 1);
      if (!existing) {
        const data = `${svg.id} ${classString(svg)} ${svg.getAttribute("aria-label") || ""} ${svg.querySelector("title")?.textContent || ""}`;
        if (/logo|brand/i.test(data) || svg.closest('header, nav, [role="banner"], [class*="logo" i], [id*="logo" i]')) {
          collect(svg, "document.querySelectorAll(svg)");
        }
      }
    });
    queryAllDeep("a[href]").filter((a) => isHomeHref(a.getAttribute("href"))).flatMap((a) => Array.from(a.querySelectorAll("img, svg"))).filter((el) => {
      const rect = el.getBoundingClientRect();
    ...
    for (const candidate of logoCandidates) {
      ...
      if (candidate.isVisible && !existing.isVisible || candidate.indicators.hrefMatch && !existing.indicators.hrefMatch || area > existingArea) {
        bySrc.set(candidate.src, candidate);
      }
    }
    const unique = Array.from(bySrc.values());
    const pickable = unique.filter((c) => c.isVisible);
    const best = (pickable.length ? pickable : unique).reduce((bestCandidate, candidate) => {
      if (!bestCandidate)
        return candidate;
      if (candidate.indicators.hrefMatch && candidate.indicators.inHeader && !(bestCandidate.indicators.hrefMatch && bestCandidate.indicators.inHeader))
        return candidate;
      if (!candidate.isSvg && bestCandidate.isSvg)
        return candidate;
      ...
      if (area >= constants.minSignificantArea && bestArea < constants.minSignificantArea)
        return candidate;
      return candidate.position.top < bestCandidate.position.top ? candidate : bestCandidate;
    }, null);
    if (best)
      pushImage(best.src, best.isSvg ? "logo-svg" : "logo");
    return { images, logoCandidates: unique };
  };
  const pickFontStack = (el) => (getStyle(el ?? document.documentElement).fontFamily || "").split(",").map((font) => font.replace(/["']/g, "").trim()).filter(Boolean);
  const getTypography = () => {
  ...
  const detectFrameworkHints = () => {
    ...
    if (scripts.some((s) => /tailwind|cdn\.tailwindcss/.test(s)))
      hints.add("tailwind");
    if (scripts.some((s) => /bootstrap/.test(s)))
      hints.add("bootstrap");
    ...
    if (/\b(px|py|mx|my|bg|text|rounded|flex|grid|items-center|justify-)/.test(classSample))
  ...
  const detectColorScheme = () => {
    ...
    const dark = html2.classList.contains("dark") || body?.classList.contains("dark") || html2.getAttribute("data-theme") === "dark" || body?.getAttribute("data-theme") === "dark" || html2.getAttribute("data-bs-theme") === "dark";
    const light = html2.classList.contains("light") || body?.classList.contains("light") || html2.getAttribute("data-theme") === "light" || body?.getAttribute("data-theme") === "light" || html2.getAttribute("data-bs-theme") === "light";
    if (dark)
      return "dark";
    if (light)
      return "light";
    const bg = getStyle(body ?? html2).backgroundColor || getStyle(html2).backgroundColor;
  ...
  const extractBrandName = () => {
    ...
    const titleBrand = titleParts.length > 1 ? titleParts[titleParts.length - 1] : title.replace(/\s*[-|–—:].*$/, "").trim();
    return site || app || titleBrand || h1 || domain || "";
  };
  const getBackgroundCandidates = () => {
    const candidates = [];
    const freq = new Map;
    for (const el of Array.from(document.querySelectorAll("body, html, main, article, [role='main'], div, section")).slice(0, constants.maxBackgroundSamples)) {
      try {
        ...
        const rect = el.getBoundingClientRect();
        const area = rect.width * rect.height;
        if (area <= constants.minSignificantArea)
          continue;
        const key = normalizeColor(bg);
        if (key)
          freq.set(key, (freq.get(key) ?? 0) + area);
      } catch (error) {
    ...
    const add = (color, source, priority, area) => {
    ...
    for (const el of Array.from(document.querySelectorAll("main, article, [role='main'], header, .main, .container")).slice(0, 5)) {
      const rect = el.getBoundingClientRect();
