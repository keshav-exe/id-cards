/**
 * Vendored from brandpull (MIT) — src/branding/processor.ts.
 * https://github.com/suraj-xd/brandpull
 * LLM enhancement pass omitted.
 */
...
function inferPalette(raw: RawBranding) {
  ...
  if (raw.pageBackground)
    bump(hexify(raw.pageBackground), 1000);
  ...
  const ranked = Array.from(freq.entries()).sort((a, b) => b[1] - a[1]).map(([color]) => color);
  let background = "#FFFFFF";
  const pageBackground = raw.pageBackground ? hexify(raw.pageBackground) : null;
  if (pageBackground && isGrayish(pageBackground))
    background = pageBackground;
  if (background === "#FFFFFF" || !raw.pageBackground && ranked.length > 0) {
    if (raw.colorScheme === "dark") {
      background = ranked.find((h) => isGrayish(h) && contrastYiq(h) < 128 && contrastYiq(h) > 0) || ranked.find((h) => isGrayish(h) && contrastYiq(h) < 180) || "#1A1A1A";
    } else {
      background = ranked.find((h) => isGrayish(h) && contrastYiq(h) > 180) || "#FFFFFF";
    }
  }
  const textPrimary = raw.colorScheme === "dark" ? ranked.find((h) => h !== background && contrastYiq(h) > 180) || "#FFFFFF" : ranked.find((h) => h.toUpperCase() !== "#FFFFFF" && contrastYiq(h) < 160) || "#111111";
  const chromatic = ranked.filter((h) => isVibrant(h) && h !== background && h !== textPrimary);
  const primary = chromatic[0] || ranked.find((h) => !isGrayish(h) && h !== textPrimary && h !== background) || (raw.colorScheme === "dark" ? "#FFFFFF" : "#000000");
  const accent = chromatic.find((h) => h !== primary) || primary;
  const secondary = chromatic.find((h) => h !== primary && h !== accent);
  const textSecondary = raw.colorScheme === "dark" ? ranked.find((h) => h !== textPrimary && contrastYiq(h) > 140 && isGrayish(h)) : ranked.find((h) => h !== textPrimary && contrastYiq(h) < 180 && isGrayish(h));
  return {
    primary,
    secondary,
    accent,
...
function inferBaseUnit(values: number[]) {
  const normalized = values.filter((v) => Number.isFinite(v) && v > 0 && v <= 128).map((v) => Math.round(v));
  if (!normalized.length)
    return 8;
  for (const candidate of [4, 6, 8, 10, 12]) {
    const share = normalized.filter((v) => v % candidate === 0 || Math.abs(v % candidate - candidate) <= 1 || v % candidate <= 1).length / normalized.length;
    if (share >= 0.6)
      return candidate;
  }
...
function pickBorderRadius(values: (number | null)[]) {
...
function fontsFromStacks(stacks: string[][]) {
  const ignored = /^(system-ui|sans-serif|serif|monospace|ui-sans-serif|ui-serif|arial|helvetica|times new roman|inherit|initial|unset)$/i;
  const freq = new Map<string, number>();
  ...
  return Array.from(freq.entries()).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([family, count]) => ({ family, count }));
}
function cleanFontName(font: string) {
  const noQuotes = font.replace(/["']/g, "").trim();
  const next = noQuotes.match(/^__(.+?)(?:_Fallback)?_[a-f0-9]+$/i);
  if (next?.[1])
    return next[1].replace(/_/g, " ").trim();
  return noQuotes.replace(/_Fallback_[a-f0-9]+$/i, "").replace(/_/g, " ").trim();
}
function representativeBorderRadius(radius: BorderRadiusCorners | null | undefined) {
  const max = Math.max(radius?.topLeft || 0, radius?.topRight || 0, radius?.bottomRight || 0, radius?.bottomLeft || 0);
  return max > 0 ? `${max}px` : "0px";
}
function corners(radius: BorderRadiusCorners | null | undefined) {
  return {
...
function snapshotToButton(snapshot: StyleSnapshot, index: number, raw: RawBranding, score: number): ButtonStyle {
  const borderHex = snapshot.colors.borderWidth && snapshot.colors.borderWidth > 0 ? hexify(snapshot.colors.border, raw.pageBackground) : null;
  return {
    index,
    text: snapshot.text || "",
...
function extractButtons(raw: RawBranding): ButtonStyle[] {
  const candidates = raw.snapshots.filter((snapshot) => {
    if (!snapshot.isButton)
      return false;
    if (snapshot.rect.w < 30 || snapshot.rect.h < 25)
      return false;
    ...
    const bg = hexify(snapshot.colors.background, raw.pageBackground);
    const border = snapshot.colors.borderWidth && snapshot.colors.borderWidth > 0 ? hexify(snapshot.colors.border, raw.pageBackground) : null;
  ...
  const unique: ButtonStyle[] = [];
  for (const item of candidates) {
    const bg = hexify(item.snapshot.colors.background, raw.pageBackground) || "transparent";
    const border = item.snapshot.colors.borderWidth && item.snapshot.colors.borderWidth > 0 ? hexify(item.snapshot.colors.border, raw.pageBackground) || "transparent-border" : "no-border";
...
function classifyButtons(buttons: ButtonStyle[]) {
  if (!buttons.length)
    return { primary: undefined, secondary: undefined, confidence: 0 };
  const scored = buttons.map((button) => {
    let score = button.score ?? 0;
    if (isVibrant(button.background))
      score += 400;
    if (/primary|brand|cta|accent|bg-brand|bg-primary/i.test(button.classes))
      score += 250;
    if (/get started|sign up|start|try|buy|book|demo|download|join/i.test(button.text))
      score += 200;
    if (button.background === "transparent" || button.background === "#FFFFFF")
      score -= 80;
    return { button, score };
  }).sort((a, b) => b.score - a.score);
  const primary = scored[0]?.button;
  const secondary = scored.find((item) => item.button.index !== primary?.index && item.button.background !== primary?.background)?.button;
  return { primary, secondary, confidence: primary ? 0.65 : 0 };
}
function extractInputs(raw: RawBranding): InputStyle[] {
  const candidates = raw.snapshots.filter((snapshot) => snapshot.isInput && snapshot.inputMetadata && snapshot.rect.w >= 50 && snapshot.rect.h >= 20).map((snapshot) => {
...
export function processRawBranding(raw: RawBranding, options: { debug?: boolean; url?: string } = {}): BrandingProfile {
  ...
  const profile: BrandingProfile = {
    ...
    typography: {
      fontFamilies: {
        ...
        heading: cleanFontName(raw.typography.stacks.heading[0] || raw.typography.stacks.body[0] || fonts[0]?.family || "system-ui")
      },
    ...
    components: {},
    images: {
      logo: raw.images.find((image) => image.type === "logo")?.src || raw.images.find((image) => image.type === "logo-svg")?.src || null,
  ...
  if (inputs[0])
    profile.components.input = inputs[0];
