/**
 * Vendored from brandpull (MIT) — src/branding/logo.ts.
 * https://github.com/suraj-xd/brandpull
 */
import type { LogoCandidate, LogoPosition, LogoSelection } from "./types"
...
function logoArea(position: LogoPosition) {
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    for (const [otherIndex, other] of candidates.entries()) {
      if (index === otherIndex || processed.has(otherIndex))
        continue;
      const sameAlt = candidate.alt && other.alt && candidate.alt.toLowerCase().replace(/\s+/g, "") === other.alt.toLowerCase().replace(/\s+/g, "");
      const sameFile = filename(candidate.src) && filename(candidate.src) === filename(other.src);
      const samePosition = Math.abs(candidate.position.top - other.position.top) < 20 && Math.abs(candidate.position.left - other.position.left) < 50 && Math.abs(candidate.position.width - other.position.width) < 30;
...
function pickBestVariant(candidates: LogoCandidate[], indices: number[]) {
  return indices.reduce((best, current) => {
    const b = candidates[best];
    const c = candidates[current];
    if (c.isVisible && !b.isVisible)
      return current;
    if (!c.isVisible && b.isVisible)
      return best;
    if (c.indicators.inHeader && !b.indicators.inHeader)
      return current;
    if (!c.indicators.inHeader && b.indicators.inHeader)
      return best;
    if (c.position.top < b.position.top)
      return current;
    if (c.position.top > b.position.top)
      return best;
    if (c.indicators.hrefMatch && !b.indicators.hrefMatch)
      return current;
    return best;
  });
}
...
export function selectLogoWithConfidence(candidates: LogoCandidate[], brandName: string): LogoSelection {
  ...
  for (const group of variants.values()) {
    ...
    if (group.some((i) => repeated.has(i)))
      variantBonus.set(best, 15);
    if (group.length > 1)
  ...
  const scored = candidates.map((candidate, index) => {
    ...
    const reasons = score > 0 ? [`variant bonus ${score}`] : [];
    ...
    if (candidates.every((other, otherIndex) => otherIndex === index || candidate.position.top <= other.position.top)) {
      score += 12;
    ...
    if (area > 1000 && area < 50000)
      ...
      score -= 20;
    if (square && candidate.position.width < 40)
      score -= 12;
    if (candidate.isSvg)
      score += 3;
